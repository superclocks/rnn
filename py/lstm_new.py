...
class LstmCell:
    ...
    def CleanDerInfos(self):
        # der
        self.dLWgx_ = np.zeros_like(self.paras_dict['Wgx_'])
        self.dLWix_ = np.zeros_like(self.paras_dict['Wix_'])
        self.dLWfx_ = np.zeros_like(self.paras_dict['Wfx_'])
        self.dLWox_ = np.zeros_like(self.paras_dict['Wox_'])
    
        self.dLWgh_ = np.zeros_like(self.paras_dict['Wgh_'])
        self.dLWih_ = np.zeros_like(self.paras_dict['Wih_'])
        self.dLWfh_ = np.zeros_like(self.paras_dict['Wfh_'])
        self.dLWoh_ = np.zeros_like(self.paras_dict['Woh_'])
    
        self.dLbg_ = np.zeros_like(self.paras_dict['bg_'])
        self.dLbi_ = np.zeros_like(self.paras_dict['bi_'])
        self.dLbf_ = np.zeros_like(self.paras_dict['bf_'])
        self.dLbo_ = np.zeros_like(self.paras_dict['bo_'])
    ...
    def Forward(self, t, xt):
        ...
        t1 = self.Calculator(self.paras_dict['Wgx_'], xt, self.paras_dict['Wgh_'], self.ht_[t - 1], self.paras_dict['bg_'])
        self.gt_.append(Tanh(t1))
        self.dgt_.append(DerTanh(t1))
        
        t2 = self.Calculator(self.paras_dict['Wix_'], xt, self.paras_dict['Wih_'], self.ht_[t - 1], self.paras_dict['bi_'])
        self.it_.append(Sigmoid(t2))
        self.dit_.append(DerSigmoid(t2))
        
        t3 = self.Calculator(self.paras_dict['Wfx_'], xt, self.paras_dict['Wfh_'], self.ht_[t - 1], self.paras_dict['bf_'])
        self.ft_.append(Sigmoid(t3))
        self.dft_.append(DerSigmoid(t3))
        
        t4 = self.Calculator(self.paras_dict['Wox_'], xt, self.paras_dict['Woh_'], self.ht_[t - 1], self.paras_dict['bo_'])
    ...
    def DLhT(self, yt, t):
        # compute time T
        h_pred = copy.deepcopy(self.ht_[t])
        h_pred -= yt
        
        # compute for time T - 1
        
        return h_pred.reshape([h_pred.shape[0], 1])
    # compute time T dLs
    def DLsT(self, yt):
        s_T = self.st_[-1] 
        o_T = self.ot_[-1]
    ...
    def Backward(self, t, xt, yt, is_last):
        #self.CleanDerInfos()
        t = type(ys)
        if(not(t is list)):
            ys = list(ys)
...
class Lstm:
    ...
    def Forward(self, xs):
        ...
        for ti in range(1, t + 1):
            xt = xs[ti - 1]
            for layer in range(len(self.net_)):
                self.net_[layer].Forward(ti, xt)
                xt = self.net_[layer].ht_[ti]
    def Backward(self, xs, ys):
        ...
        for ti in range(t, 1, -1):
            xt = xs[t]
            yt = ys[t]
            for layer in range(len(self.net_) - 1, -1, -1):
                self.net_[layer].Bachward(ti, xt, yt, is_last)
            is_last = False
    ...
    def Train(self):
        for i in range(len(self.X_)):
            xs = self.X_[i]
            ys = self.Y_[i]
            # clean all fowward infos in net
            for net in self.net_:
                net.CleanDerInfos()
                net.CleanForwardInfos()            
            self.Forward(xs)
#class Lstm:
    #def __init__(self, num_input, num_hidden, num_embeding, num_layer, num_in_dict, num_out_dict):
